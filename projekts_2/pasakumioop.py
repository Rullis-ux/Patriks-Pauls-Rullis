...
#abstraktā bāzes klases izveide
class Pasakums(ABC):
...
class Festivals(Pasakums):
    def aprekinat_cenu(self,biletes_skaits):
        ...
        bezmaksas = biletes_skaits // 4
        #if biletes_skaits in bilete:
        return self.cena*(biletes_skaits-bezmaksas)
...
class PasakumuPlanotajs:
    def __init__(self):
        self.pasakumi = {
            ...
            'sacensibas' : Sacensibas('sacensibas',[date(2025, 7, 30), date(2025, 9, 2), date(2025, 11, 17)],120,12)
        }
    ...
    def izveleties_pasakumu(self):
       while True:
            try:
                ...
                if ievade in veidi:
                    return self.pasakumi[veidi[ievade]]
                else:
    ...
    def izveleties_biletes(self,pasakums):#while true #except
        while True:
            try:
                ...
                bilesu_sk = int(input('Cik biļetes vēlaties?(0-beigt programmu, ja vairs nav biļešu): '))
                #nevar būt < par 0
                if 0< bilesu_sk <=pasakums.brivas_vietas():
                    pasakums.ietilpiba-=bilesu_sk
                    return bilesu_sk
                elif bilesu_sk==0:
    ...
    def sakt(self):
        print('Laipni lūgti pasākumu plānotājā!')
        while True:
            pasakums=self.izveleties_pasakumu()
            izveletais_datums=self.izveleties_datumu(pasakums)
            ...
            print(f'Pasākuma veids:  {pasakums.nosaukums}') #nosaukums
            print(f'Datums: {izveletais_datums}')
            print('Dienas līdz pasākumam: ',pasakums.dienas_lidz_pasakumam(izveletais_datums))
            print('Kopējā biļēšau cena: ', pasakums.aprekinat_cenu(biletes), 'EUR')
            print('\nPaldies par iepirkšanos!')

            turpinat=input("Vai vēlaties turpināt programmu? (1-jā; 2-nē): ")
            if turpinat == 2:
